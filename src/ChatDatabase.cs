...
namespace sharpkappa {
    class ChatDatabase {
        ...
        public void generateChatDatabase(string channel) {
            openConnection();
            string query = $@"CREATE TABLE IF NOT EXISTS '{channel}'(timestamp INTEGER, username TEXT, message TEXT, channel TEXT, subscriber INTEGER, game_name STRING, viewer_count INTEGER)";
        ...
        public void appendMessage(ChatMessage chatmessage) {
            if(generated) {
                openConnection();
                string query = $"INSERT INTO '{chatmessage.channel}'(timestamp, username, message, channel, subscriber, game_name, viewer_count) VALUES (@timestamp, @username, @message, @channel, @subscriber, @game_name, @viewer_count)";
                SQLiteCommand sqlcmd = new SQLiteCommand(query, connection);
                ...
                sqlcmd.Parameters.AddWithValue("@subscriber", chatmessage.subscriber);
